...
export default function GroupCard({ group, tasks }) {
  const participantCount = group.students?.length || 1 + (group.participants?.length || 0);
  ...
  const maxHP = group.monster_hp || totalTaskPoints;
    ...
    <Link to={`/group/${group.id}`}>
      ...
      >
        ...
        <div 
          ...
          style={{ backgroundColor: group.color || '#6B21A8' }}
        />
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-slate-500">
              ...
              <span>{group.monster_name || 'Monster'}</span>
            </div>
          </div>
          ...
          {isDead ? (
            ...
            <div 
              ...
              style={{ backgroundColor: `${group.color || '#6B21A8'}20` }}
            >
              <Sword className="w-6 h-6" style={{ color: group.color || '#6B21A8' }} />
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="font-mono font-medium" style={{ color: group.color || '#6B21A8' }}>
                {currentHP} / {maxHP}
              </span>
            </div>
            <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
              <motion.div
                ...
                style={{ backgroundColor: isDead ? '#22C55E' : (group.color || '#6B21A8') }}
                initial={{ width: 0 }}
